...
const getWebkitBuildPaths = async (
  args: Namespace,
): Promise<WebkitBuildPaths | undefined> => {
  if (args.webkit_build === undefined) {
    return undefined;
  }
  const wkBuildDir: Path = normalize(String(args.webkit_build));

  const webkitPaths: WebkitBuildPaths = {
  ...
  if (!(await isPathToReadableDir(wkBuildDir))) {
    throw new Error(
      "Arg for --webkit-build (-w) is not a readable directory: " + wkBuildDir,
    );
  }

  if (args.browser !== BrowserType.WebKit) {
    throw new Error(
      "The --webkit-build-dir (-w) argument can only be used when the " +
  ...
  if (args.binary_path !== undefined) {
    throw new Error(
      "Cannot use the --webkit-build (-w) argument and the --binary (-x) " +
  ...
  if (!(await isPathToReadableDir(webkitPaths.releaseDir))) {
    throw new Error(
      "The given --webkit-build (-w) directory does not have the expected " +
...
const makeOutputHandle = async (
  outputArg: undefined | Path,
  url: URL,
): Promise<Writable> => {
...
export const runConfigForArgs = async (args: Namespace): Promise<RunConfig> => {
  ...
  let binaryPath = args.binary_path as Path | undefined;
  ...
  if (binaryPath === undefined) {
    ...
    if (!(await isPathToExecFile(binaryPath))) {
      throw new Error(
        `Invalid binary path. "${binaryPath}" is not an ` + "executable file.",
      );
    }
  }
