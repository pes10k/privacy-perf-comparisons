...
const getContext = async (browser: BrowserType, userDataDir: Path,
                          launchArgs: LaunchArgs): Promise<BrowserContext> => {
  const browserType = browserTypeMapping[browser]
  assert(browserType)
  return await browserType.launchPersistentContext(userDataDir, launchArgs)
}

export const launch = async (logger: Logger, config: RunConfig): Promise<BrowserContext> => {
