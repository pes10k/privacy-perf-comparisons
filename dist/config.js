...
export const runConfigForArgs = async (args) => {
    ...
    const isChromium = (args.browser === BrowserType.Chromium
        || args.browser === BrowserType.Brave);
    if (args.profile && isChromium === false) {
    ...
    const isCaseTwo = (!isCaseOne
        && args.userDataDir
        && !isUserDataDirExisting);
    const isCaseThree = (!isCaseOne && !isCaseTwo
        && !isChromium
        && isUserDataDirExisting);
    const isCaseFour = (!isCaseOne && !isCaseTwo && !isCaseThree
        && isChromium
        && isUserDataDirExisting
    ...
    if (args.binary_path === undefined) {
        ...
        switch (args.browser) {
            case [BrowserType.Brave]:
            case [BrowserType.Chromium]:
                if ((await isPathToExecFile(args.binary_path)) === false) {
